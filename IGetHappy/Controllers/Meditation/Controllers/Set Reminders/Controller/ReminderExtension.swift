            ...
            if firstDate != nil && lastDate == nil
            {
                ...
                else
                {
                    if(range.count > Singleton.shared().totalDays)
                    {
                        ...
                        CommonVc.AllFunctions.showAlert(message: "You can not select days more than \(String(describing: Singleton.shared().setGoalDays))", view: self, title: "Oops!")
                    }
                    else
                    {
                        datesRange = range
...
/*     @objc func donedatePicker_recurring_startDate()
    ...
    func HANDLE_ADD_REMINDER_ACTION()
    {
        ...
        {
            CommonVc.AllFunctions.showAlert(message: "Please click on Select Button and choose your reminder time.", view: self, title: "Oops!")
        }
    ...
    func SAVE_RECURRING_REMINDER_IN_DATABASE(tbID:String,reminderType:String,notif_id:String,recurringDay:Int,recurring_approach:String)
    {
        DispatchQueue.main.async {
            ...
            {
                self.tf_recurringMsg.text = "iGetHappy"
            }
